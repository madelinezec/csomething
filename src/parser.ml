...
let parseT tokenlist = 
  ...
  | Lexer.LeftParens -> let token(list_f, expr_f) = parseF tokenlist in 
                let (tokenlist_tprime, expr_tprime) = parseTprime tokenlist_f in 
                (tokenlist_tprime, Ast.F(expr_f, expr_tprime))
  | Lexer.Numeral -> let token(list_f, expr_f) = parseF tokenlist in 
                let (tokenlist_tprime, expr_tprime) = parseTprime tokenlist_f in 
                (tokenlist_tprime, Ast.Literal(expr_f, expr_tprime))
  | Lexer.Bool -> let token(list_f, expr_f) = parseF tokenlist in 
                let (tokenlist_tprime, expr_tprime) = parseTprime tokenlist_f in 
                (tokenlist_tprime, Ast.F(expr_f, expr_tprime))
  | Lexer.Bool -> let token(list_f, expr_f) = parseF tokenlist in 
                let (tokenlist_tprime, expr_tprime) = parseTprime tokenlist_f in 
                (tokenlist_tprime, Ast.F(expr_f, expr_tprime))
  | Lexer.ID identifier -> let token(list_f, expr_f) = parseF tokenlist in 
                let (tokenlist_tprime, expr_tprime) = parseTprime tokenlist_f in 
                (tokenlist_tprime, Ast.F(expr_f, expr_tprime))
  | _-> let err_msg = Printf.sprintf "Syntax Error" in
        raise (Syntax_error err_msg)
...
let parseF tokenlist = 
  match tokenlist.lookathead with
  | Lexer.LeftParens -> let (tokenlist_expr, expr) = next tokenlist |> parseE in 
                match next tokenlist_expr with 
                | Lexer.RightParens -> (next tokenlist_expr, Ast.F(expr))
