...
Mat* mat_inv(Mat* m) {
    ...
    if (m->type == 1) 
        return (Mat*)mat_inverse_float((Mat_f*) m);
    else 
        return (Mat*)mat_inverse_int((Mat_i*) m);
}
...
Mat_f* mat_inverse_float(Mat_f* mat){
    void_check((void*) mat);
    if(mat->m != mat->n){
		char str[] = "Matrix Cannot Be Inversed";
		write(STD_ERR, str, strlen(str));
		exit(-1);
	}
	
	Mat_f* mat_result = alloc_mat_float(mat->n, mat->n);
    matrix_inversion_helper(mat->data, mat->n, mat_result->data);
...
Mat_f* mat_inverse_int(Mat_i* mat){
	...
	Mat_f* mat_f =  mat_int_to_float(mat);
	Mat_f* mat_result = alloc_mat_float(mat->n, mat->n);
    matrix_inversion_helper(mat_f->data, mat->n, mat_result->data);
    free_matrix_float(mat_f);
...
//matrix inverse
void matrix_inversion_helper(float** A, int order, float** Y)
{
    ...
    float *temp = (float*) malloc((order-1)*(order-1)*sizeof(float));
    //float **minor = new float*[order-1];
    float **minor = (float**) malloc((order-1)*sizeof(float*));
...
float CalcDeterminant( float **mat, int order)
{
    ...
    float **minor = (float**) malloc((order-1)*sizeof(float*));
    for(int i=0;i<order-1;i++)
        minor[i] = (float*) malloc((order-1)*sizeof(float));
